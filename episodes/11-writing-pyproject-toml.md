...
The first step in packaging your project is to create a  `pyproject.toml` file. In your editor of choice, at the top level of the repository, create a new file called `pyproject.toml`.  In the file, add the following, changing the name of the project appropriately:
...
These are added to the `[project]` section of the `pyproject.toml`:
``` toml
[project]
name = "packaging-project-[your-name]"
version = "0.0.1"
...
The project has a GitHub Action for running tests. As currently written it just installs every dependency by name.  We can now update it to install using the `pyproject.toml`.  We can change the line `pip install numpy pillow torch 'torchvision >= 0.13' tqdm` to simply `pip install -e .`, and the line `pip install black flake8 isort` to `pip install --group dev`.
...
Right now if we want to run the CLI tool or GUI app we need to run it using `python -m` to run the module as the main script.  The `pyproject.toml` provides a way to let Python know that certain code is meant to be run as a command-line script.

You can add a `[project.scripts]` section which lists command-line scripts:
``` toml
[project.scripts]
...
For GUIs you need to instead use:
``` toml
[project.gui-scripts]
...
After you have set up the information about the code and dependencies, generally you will want to add a lot more metadata about your project.  For example you might add the following to the `[project]` section (updating the authors and maintainers sections appropriately):
...
For most of these tools, this can be inside the `pyproject.toml` file itself.  For example our project currently has an `.isort.cfg` file which looks like this:
``` ini
[settings]
profile = black
line_length = 88
...
Unfortunately `flake8` doesn't currently support this sort of usage of `pyproject.toml` yet.
